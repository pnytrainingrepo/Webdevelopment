...
function getExchangeRate(fromCurrency: string, toCurrency: string): number {
  ...
  console.log(
    chalk.magentaBright(`Exchange Rate: 1 ${fromCurrency} = ${chalk.yellow(exchangeRate)} ${toCurrency}`)
  );

  console.log(
    chalk.greenBright(`Converted Amount: ${chalk.yellow(convertedAmount.toFixed(2))} ${toCurrency}\n`)
  );
